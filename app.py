...
def flexible_model(mines, ports, years, plump, exp_cost, haulage_rate, discount_rate):
    ...
    for year in range(years + 1):
        ...
        if system_demand <= system_capacity:
            ...
            # fixed with reroute
            caps_fixed = cap_prior.copy()
            for p in ports:
                caps_fixed[p.name] += fixed_chunks * plump
            assign_fixed = {k: dict(v) for k, v in assignments.items()}
            usage_fx = usage.copy()
            for p in ports:
                if usage_fx[p.name] > caps_fixed[p.name]:
                    others = [o for o in ports if usage_fx[o.name] < caps_fixed[o.name]]
                    items_fx = sorted(
                        assign_fixed[p.name].items(),
                        key=lambda mv: ((next(m for m in mines if m.name == mv[0]).distance_to_dbct if p.name == 'DBCT' else next(m for m in mines if m.name == mv[0]).distance_to_appt) * haulage_rate)
                    )
                    for mn, vol in items_fx:
                        if usage_fx[p.name] <= caps_fixed[p.name]: break
                        mv_fx = min(vol, usage_fx[p.name] - caps_fixed[p.name])
                        assign_fixed[p.name][mn] -= mv_fx; usage_fx[p.name] -= mv_fx
                        assign_fixed[others[0].name][mn] = assign_fixed[others[0].name].get(mn, 0) + mv_fx; usage_fx[others[0].name] += mv_fx
...
# --- NPV calculation ---
def compute_npv(series, rate): return sum(series[t] / ((1 + rate) ** t) for t in series.index)
...
with st.sidebar:
    ...
    base_df = pd.DataFrame({"Name": ["Mine 1","Mine 2"],"Distance to DBCT": [80,130],"Output0": [500,500],"Growth rate": [0.05,0.05]})
    try: mines_df = st.data_editor(base_df, use_container_width=True)
    except: mines_df = st.experimental_data_editor(base_df, use_container_width=True)
    if st.button("Run simulation"): st.session_state.run = True
...
if st.session_state.run:
    ...
    for title, diff in [("Cumulative APPT Cost Difference ($)", df_f['appt_cost'].cumsum() - df_x['appt_cost'].cumsum()),
                        ("Cumulative Haulage Cost Difference ($)", df_f['haulage_cost'].cumsum() - df_x['haulage_cost'].cumsum()),
                        ("PV of Cost Differences Over Time ($)", pd.Series([sum((df_f['appt_cost']-df_x['appt_cost']).iloc[:t+1]/((1+DISCOUNT_RATE)**np.arange(t+1)))+
                                     sum((df_f['haulage_cost']-df_x['haulage_cost']).iloc[:t+1]/((1+DISCOUNT_RATE)**np.arange(t+1)))
                                     for t in range(YEARS+1)]))]:
        st.subheader(title)
        df_plot = pd.DataFrame({"Year": diff.index if hasattr(diff, 'index') else range(YEARS+1), "Diff": diff.values if hasattr(diff, 'values') else diff})
        chart = alt.Chart(df_plot).mark_bar() if "APPT" in title else alt.Chart(df_plot).mark_line(point=True)
        st.altair_chart(chart.encode(x='Year:O' if "APPT" in title else 'Year:Q', y='Diff:Q'), use_container_width=True)
