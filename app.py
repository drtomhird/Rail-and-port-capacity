    ...
    for year in range(years + 1):
        ...
        for p in ports:
            if usage[p.name] > p.capacity:
                ...
                if p.name == 'DBCT': dbct_cost += chunks * exp_cost
                else: appt_cost += chunks * exp_cost
    ...
    for year in range(years + 1):
        ...
        for p in ports:
            if usage_after[p.name] > p.capacity:
                ...
                items = sorted(assignments[p.name].items(), key=lambda mv: ((next(m for m in mines if m.name==mv[0]).distance_to_dbct if p.name=='DBCT' else next(m for m in mines if m.name==mv[0]).distance_to_appt)*haulage_rate))
                for mine_name, vol in items:
                    if usage_after[p.name] <= p.capacity: break
                    ...
                    usage_after[p.name]-=move_vol; usage_after[other.name]+=move_vol
        haulage = compute_haulage_costs(assignments, haulage_rate, mines)
        total = dbct_cost + appt_cost + haulage
        results['dbct_cost'][year] = dbct_cost
        results['appt_cost'][year] = appt_cost
