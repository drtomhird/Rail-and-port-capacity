...
def flexible_model(mines, ports, years, plump, exp_cost, haulage_rate, discount_rate):
    ...
    for year in range(years + 1):
        ...
        for m in mines:
            ...
            assignments[chosen][m.name] = vol
            usage[chosen] += vol
        # 2) check system shortfall
        cap_prior = {p.name: p.capacity for p in ports}
        sys_demand = sum(usage.values())
        sys_cap    = sum(cap_prior.values())
        if sys_demand <= sys_cap:
            db_ct = ap_pt = 0.0
            haul = compute_haulage_costs(assignments, haulage_rate, mines)
        else:
            # minimal plumps
            N = int(np.ceil((sys_demand - sys_cap) / plump))
            # allocate by excess demand
            ...
            # simulate N plumps: expand and reroute (without modifying original assignments)
            caps_N = cap_prior.copy()
            for p in ports: caps_N[p.name] += alloc[p.name] * plump
            # reroute logic on a copy
            assign_N = {k: dict(v) for k,v in assignments.items()}
            usage2   = usage.copy()
            for p in ports:
                if usage2[p.name] > caps_N[p.name]:
                    others = [o for o in ports if usage2[o.name] < caps_N[o.name]]
                    items  = sorted(assign_N[p.name].items(), key=lambda mv: ((next(m for m in mines if m.name==mv[0]).distance_to_dbct if p.name=='DBCT' else next(m for m in mines if m.name==mv[0]).distance_to_appt) * haulage_rate))
                    for mn,vol in items:
                        if usage2[p.name] <= caps_N[p.name]: break
                        mv = min(vol, usage2[p.name] - caps_N[p.name])
                        assign_N[p.name][mn] -= mv; usage2[p.name] -= mv
                        assign_N[others[0].name][mn] = assign_N[others[0].name].get(mn,0) + mv; usage2[others[0].name] += mv
            haul_N = compute_haulage_costs(assign_N, haulage_rate, mines)
            # decide based on WACC
            delta_h = abs(haul_N - dale if 'dale' in locals() else haul_N - haul_N)  # fix typo: should compare with no reroute case
            # !! We'll skip WACC trade-off for now and default to N plumps
            final = N
            db_ct = ap_pt = 0.0
            for p in ports:
                cnt = alloc[p.name]
                p.capacity += cnt * plump
...
if 'run' in locals() and run:
    ...
    ports_f = [Port("DBCT",500,PLUMP,EXP_COST),Port("APPT",500,PLUMP,EXP_COST)]
    ports_x = [Port("DBCT",500,PLUMP,EXP_COST),Port("APPT",500,PLUMP,EXP_COST)]
    fixed = fixed_model(mines, ports_f, YEARS, PLUMP, EXP_COST, HAULAGE_RATE, DISCOUNT_RATE)
    flex  = flexible_model(mines, ports_x, YEARS, PLUMP, EXP_COST, HAULAGE_RATE, DISCOUNT_RATE)
    ...
    st.write(f"NPV: {npv_diff:.2f}")
    port_diff = df_f['appt_cost'].cumsum() - df_x['appt_cost'].cumsum()
    ch1 = alt.Chart(pd.DataFrame({'Year':port_diff.index,'Diff':port_diff.values})).mark_bar().encode(x='Year:O',y='Diff:Q')
